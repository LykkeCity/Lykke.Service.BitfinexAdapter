...
namespace Lykke.Service.BitfinexAdapter.Controllers.Api
{
    public class ExchangeController : BaseApiController
    {
        ...
        {
            return Ok(_configuration.UseSupportedCurrencySymbolsAsFilter ? _configuration.SupportedCurrencySymbols.Select(s => s.ExchangeSymbol).ToList() : await GetUnAuthenticatedExchange().GetAllExchangeInstruments());
        }
    }
}
